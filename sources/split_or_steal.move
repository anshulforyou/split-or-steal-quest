...
module overmind::split_or_steal {
    ...
    */
    fun init_module(account: &signer) {
        ...
        move_to(account, State{
            ...
            cap: signer_cap,
            create_game_events: account::new_event_handle<CreateGameEvent>(&resource),
            submit_decision_events: account::new_event_handle<SubmitDecisionEvent>(&resource),
            reveal_decision_events: account::new_event_handle<RevealDecisionEvent>(&resource),
            conclude_game_events: account::new_event_handle<ConcludeGameEvent>(&resource),
            release_funds_after_expiration_events: account::new_event_handle<ReleaseFundsAfterExpirationEvent>(&resource)
        });
    }
    ...
    */
    public entry fun create_game(
        ...
        let game_id = get_next_game_id(&mut borrow_global_mut<State>(@overmind).next_game_id);
        ...
        simple_map::add(&mut borrow_global_mut<State>(@overmind).games, game_id, game);
        // TODO: Transfer `prize_pool_amount` amount of APT from `account` to the resource account
        let resource_address = account::get_signer_capability_address(&borrow_global_mut<State>(@overmind).cap);
        coin::transfer<AptosCoin>(account, resource_address, prize_pool_amount);
        // TODO: Emit `CreateGameEvent` event
        event::emit_event<CreateGameEvent>(&mut borrow_global_mut<State>(@overmind).create_game_events, CreateGameEvent{
            game_id: game_id,
            prize_pool_amount: prize_pool_amount,
    ...
    */
    public entry fun submit_decision(
        ...
        check_if_game_exists(&borrow_global<State>(@overmind).games, &game_id);
        // TODO: Call `check_if_player_participates_in_the_game` function
        check_if_player_participates_in_the_game(player, &*simple_map::borrow(&borrow_global<State>(@overmind).games, &game_id));
        // TODO: Call `check_if_player_does_not_have_a_decision_submitted` function
        check_if_player_does_not_have_a_decision_submitted(&*simple_map::borrow(&borrow_global<State>(@overmind).games, &game_id), signer::address_of(player));
        // TODO: Set the player's PlayerData decision_hash and salt_hash fields to the values provided in the params
        let game = simple_map::borrow_mut(&mut borrow_global_mut<State>(@overmind).games, &game_id);
        let player_address = signer::address_of(player);
        if (game.player_one.player_address == player_address){
            game.player_one.decision_hash = option::some(decision_hash);
            game.player_one.salt_hash = option::some(salt_hash);
        } else if (game.player_two.player_address == player_address){
            game.player_two.decision_hash = option::some(decision_hash);
            game.player_two.salt_hash = option::some(salt_hash);
        } else {
            abort(EPlayerDoesNotParticipateInTheGame)
        };
        // TODO: Emit `SubmitDecisionEvent` event
        event::emit_event<SubmitDecisionEvent>(&mut borrow_global_mut<State>(@overmind).submit_decision_events, SubmitDecisionEvent{
            game_id: game_id,
    ...
    */
    public entry fun reveal_decision(
        ...
        check_if_both_players_have_a_decision_submitted(&*simple_map::borrow(&borrow_global<State>(@overmind).games, &game_id));
        ...
        let decision;
        if (game.player_one.player_address == player_address){
            decision = make_decision(&mut game.player_one, &salt);
        } else if (game.player_two.player_address == player_address){
            decision = make_decision(&mut game.player_two, &salt);
        } else {
            abort(EPlayerDoesNotParticipateInTheGame)
        };
        ...
        if (game.player_one.decision != DECISION_NOT_MADE && game.player_two.decision != DECISION_NOT_MADE){
            let deployed_signer = account::create_signer_with_capability(&state.cap);
            let (_, game) = simple_map::remove(&mut state.games, &game_id);
            if (game.player_one.decision == DECISION_SPLIT && game.player_two.decision == DECISION_SPLIT){
                coin::transfer<AptosCoin>(&deployed_signer, game.player_one.player_address, game.prize_pool_amount / 2);
                coin::transfer<AptosCoin>(&deployed_signer, game.player_two.player_address, game.prize_pool_amount / 2);
            } else if (game.player_one.decision == DECISION_STEAL && game.player_two.decision == DECISION_SPLIT){
                coin::transfer<AptosCoin>(&deployed_signer, game.player_one.player_address, game.prize_pool_amount);
            } else if (game.player_one.decision == DECISION_SPLIT && game.player_two.decision == DECISION_STEAL){
                coin::transfer<AptosCoin>(&deployed_signer, game.player_two.player_address, game.prize_pool_amount);
            } else if (game.player_one.decision == DECISION_STEAL && game.player_two.decision == DECISION_STEAL){
                coin::transfer<AptosCoin>(&deployed_signer, @overmind, game.prize_pool_amount);
            } else {
                abort(EIncorrectHashValue)
            };
    ...
    */
    public entry fun release_funds_after_expiration(_account: &signer, game_id: u128) acquires State {
        ...
        let (_, game) = simple_map::remove(&mut borrow_global_mut<State>(@overmind).games, &game_id);
        ...
        let deployed_signer = account::create_signer_with_capability(&borrow_global_mut<State>(@overmind).cap);
        if (game.player_one.decision == DECISION_NOT_MADE && game.player_two.decision == DECISION_NOT_MADE){
            coin::transfer<AptosCoin>(&deployed_signer, @overmind, game.prize_pool_amount);
        } else if (game.player_one.decision == DECISION_NOT_MADE && game.player_two.decision != DECISION_NOT_MADE){
            coin::transfer<AptosCoin>(&deployed_signer, game.player_two.player_address, game.prize_pool_amount);
        } else if (game.player_one.decision != DECISION_NOT_MADE && game.player_two.decision == DECISION_NOT_MADE){
            coin::transfer<AptosCoin>(&deployed_signer, game.player_one.player_address, game.prize_pool_amount);
        } else {
        ...
        event::emit_event<ReleaseFundsAfterExpirationEvent>(&mut borrow_global_mut<State>(@overmind).release_funds_after_expiration_events, ReleaseFundsAfterExpirationEvent{
            game_id: game_id,
            player_one_decision: game.player_one.decision,
            player_two_decision: game.player_two.decision,
            prize_pool_amount: game.prize_pool_amount,
    ...
    */
    inline fun make_decision(player_data: &mut PlayerData, salt: &String): u64 {
        ...
        // let decision_vector = bcs::to_bytes(&player_data.decision);
        // vector::append(&mut decision_vector, bcs::to_bytes(salt));
        // let player_salt_hash = option::borrow(&player_data.salt_hash);
        ...
        if (split_hash == *option::borrow(&player_data.decision_hash)) {
            player_data.decision = DECISION_SPLIT;
            DECISION_SPLIT
        } else if (steal_hash == *option::borrow(&player_data.decision_hash)) {
    ...
    */
    ...
    inline fun check_if_player_does_not_have_a_decision_submitted(game: &Game, player_address: address) {
        ...
        assert!(if (player_address == game.player_one.player_address) {
            option::is_none(&game.player_one.decision_hash)
        } else if (player_address == game.player_two.player_address) {
            option::is_none(&game.player_two.decision_hash)
        } else {
            abort(EPlayerDoesNotParticipateInTheGame)
        }, EPlayerHasDecisionSubmitted)
    }
